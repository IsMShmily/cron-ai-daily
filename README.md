...
本项目已使用 **pnpm workspace** 管理前后端，只需要在根目录安装一次依赖：
...
- 启动后端：`node server/index.js`（监听 `http://127.0.0.1:3846`，可通过环境变量 `PORT` 修改）
- 启动前端：`web` 包中的 Vite 开发服务器（默认 `http://localhost:5173`）

前端通过代理访问后端 API，无需额外配置。

### 3. 单独启动（可选）

如果你只想单独启动其中一部分，也可以使用：
